...
# Models with adjusted dropout rates
models = {
...
def train_and_evaluate_model(optimizer_name, optimizer, model_name, layers, batch_size):
    ...
    print(f"Training model with {model_name} and optimizer: {optimizer_name}, Batch Size: {batch_size}")
    ...
    results.append((optimizer_name, model_name, batch_size, stopped_epoch, final_accuracy, final_loss, final_val_loss, final_val_accuracy))

batch_sizes = [4, 8, 16]
for optimizer_name, optimizer in optimizers.items():
    for model_name, layers in models.items():
        for batch_size in batch_sizes:
            train_and_evaluate_model(optimizer_name, optimizer, model_name, layers, batch_size)

# Save results to a file
with open('model_training_results.txt', 'w') as f:
    for result in results:
        f.write(f"Optimizer: {result[0]}, Model: {result[1]}, Batch Size: {result[2]}, Stopped Epoch: {result[3]}, "
                f"Final Accuracy: {result[4]:.4f}, Final Loss: {result[5]:.4f}, "
                f"Final Validation Loss: {result[6]:.4f}, Final Validation Accuracy: {result[7]:.4f}\n")

# Display results
for result in results:
    print(f"Optimizer: {result[0]}, Model: {result[1]}, Batch Size: {result[2]}, Stopped Epoch: {result[3]}, "
          f"Final Accuracy: {result[4]:.4f}, Final Loss: {result[5]:.4f}, "
          f"Final Validation Loss: {result[6]:.4f}, Final Validation Accuracy: {result[7]:.4f}")
